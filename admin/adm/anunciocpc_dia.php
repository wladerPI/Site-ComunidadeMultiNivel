...
if ($dia == "" ||  $dia == 0 ||  $dia > 30) {
	header("Location: anuncio_cpc.php");
	exit;
}
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela16 WHERE DIA = '$dia' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	
	$sql_conta = $con->prepare("SELECT * FROM $tabela15 WHERE DIA = '$dia'");
	$sql_conta->execute();
	$sql_conta = $sql_conta->fetchAll(PDO::FETCH_OBJ);
	...
	echo "<br><br>"; // Vai servir só para dar uma linha de espaço entre a paginação e o conteúdo
				...
				foreach($sql_verifc as $ln_verifc) { 
					...
					$texto = html_entity_decode((string)$ln_verifc->ANUNCIO_CPC, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
?>
	<tbody>
        <tr> 
						<td class='class1'><?php echo $ln_verifc->ID; ?></td> 
						<td class='class2'><?php echo $ln_verifc->DIA; ?></td> 
						<td class='class2'> <?php echo $ln_verifc->ANUNCIO_TITULO; ?></td>
						<td class='class2'> <?php echo $texto; ?></td>
						<td class='class2'> <?php echo $data; ?></td>
						<td class='class2'>  
						<a class="btn btn-app" href="anuncioscpc_editar.php?id_anunciocpc=<?php echo $ln_verifc->ID; ?>&dia=<?php echo $dia; ?>" title="Editar essa An&uacute;cio">
						<i class="fa fa-edit"></i> Editar Dica
						</a> 
						
						<a class="btn btn-app" href="anuncioscpc_exclui.php?id_anunciocpc=<?php echo $ln_verifc->ID; ?>&dia=<?php echo $dia; ?>" title="Excluir essa Dica">
