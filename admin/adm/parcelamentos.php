...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela12 WHERE STATUS = 'PENDENTE' ORDER BY DATA_CADASTRO ASC LIMIT $inicial, $numreg");
	$sql->execute();
...
?>
	<tbody>
        <tr> 
						...
						<td class='class2'>
						<form id="form1" name="form1" method="post" action="parcelamento_pagando.php"> 
								<INPUT TYPE="hidden" NAME="id_parcelamento" VALUE="<?php echo $id_parcelamento; ?>">
								<INPUT TYPE="hidden" NAME="id_do_cliente" VALUE="<?php echo $id_do_cliente; ?>"> 
								<button type="submit" class="btn btn-success btn-sm" title="ATIVAR essa Solicita&ccedil;&atilde;o">PAGAR</button> 
						</form>
						</td> 
						<td class='class2'> 
						<form id="form_cancela_1" name="form_cancela_1" method="post" action="parcelamento_cancela.php">
							<INPUT TYPE="hidden" NAME="id_parcelamento" VALUE="<?php echo $id_parcelamento; ?>">
							<INPUT TYPE="hidden" NAME="id_do_cliente" VALUE="<?php echo $id_do_cliente; ?>"> 
