            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Pomo&ccedil;&atilde;o Premiados PENDENTES</li>
                    </ol>
                </section>
                <!-- Main content -->
...
<!-- centro -->		 
 <div class="box-header">
		<h3 class="box-title">Pomo&ccedil;&atilde;o Premiados PENDENTES</h3>
</div><!-- /.box-header -->	 
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela23 WHERE STATUS = 'PENDENTE' ORDER BY DATA ASC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	$quantreg_pendentes = count( $sql_verifc );
	 
	
	$sql_conta_repetidos = $con->prepare("SELECT * FROM $tabela23 WHERE STATUS = 'PENDENTE'");
...
?> 
	...
	<div class="alert alert-warning alert-dismissable">
        ...
        <b> No momento tem (<?php echo $qts_premiados_pendentes; ?>) Solicita&ccedil;&otilde;es de trocas de pontos para ADPacks </b>
    </div>
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>  
												...
												<th>Quantidade de ADPacks</th> 
												<th>Conclu&iacute;r Pagamento</th> 
                                                <th>Cancelar Troca</th> 
	...
	echo "<br><br>"; // Vai servir só para dar uma linha de espaço entre a paginação e o conteúdo
				...
				foreach($sql_verifc as $ln_verifc) { 
					...
					$qts_adpacks = $ln_verifc->QTS_ADPACKS; 
...
?>
	<tbody>
        <tr> 
						...
						<td <?php if ($advr >= 1) { ?>   style='background:#dea48b;' <?php } ?> > <?php echo $qts_adpacks; ?></td> 
						<td <?php if ($advr >= 1) { ?>   style='background:#dea48b;' <?php } ?>>
						<form id="form1" name="form1" method="post" action="promocao_pago.php"> 
								...
								<button type="submit" class="btn btn-success btn-sm" title="Clique Aqui Somente se o ADPack j&aacute; foi PAGO para esse premiado">Completar Troca</button> 
						</form>
						</td>
						<td  <?php if ($advr >= 1) { ?>   style='background:#dea48b;' <?php } ?>> 
						<form id="form_cancela_1" name="form_cancela_1" method="post" action="promocao_cancela.php">
							<INPUT TYPE="hidden" NAME="id_promocao" VALUE="<?php echo $id_promocao; ?>">
							<INPUT TYPE="hidden" NAME="id_do_cliente" VALUE="<?php echo $id_do_cliente; ?>">
							<INPUT TYPE="hidden" NAME="qts_adpacks" VALUE="<?php echo $qts_adpacks; ?>">
							<button type="submit"  class="btn btn-danger btn-sm" title="Clique aqui Somente se o premiado tenha atingido outro n&iacute;vel de premia&ccedil;&atilde;o"  >Cancelar Troca</button> 
						</form> 
						</td>
						
						<td <?php if ($advr >= 1) { ?>   style='background:#dea48b;' <?php } ?>> 
						<form id="form_adv" name="form_adv" method="post" action="promocao_advertencia.php">
							<INPUT TYPE="hidden" NAME="id_promocao" VALUE="<?php echo $id_promocao; ?>">
							<INPUT TYPE="hidden" NAME="id_do_cliente" VALUE="<?php echo $id_do_cliente; ?>">
							<INPUT TYPE="hidden" NAME="qts_adpacks" VALUE="<?php echo $qts_adpacks; ?>">
						...
						<td <?php if ($advr >= 1) { ?>   style='background:#dea48b;' <?php } ?>> <?php echo $ln_verifc->DATA; ?></td> 
					</tr>	
    </tbody>  	 
				<?php }  ?> 
<tfoot> 
