            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Solicita&ccedil;&otilde;es PENDENTES da TrafficMonsoon</li>
                    </ol>
                </section>
                <!-- Main content -->
...
<!-- centro -->		 
 <div class="box-header">
		<h3 class="box-title">Todas Solicita&ccedil;&otilde;es PENDENTES da TrafficMonsoon</h3>
</div><!-- /.box-header -->	 
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela22 WHERE STATUS = 'PENDENTE' ORDER BY DATA_CADASTRO ASC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	$quantreg_pendentes = count( $sql_verifc );
	
	$sql_conta = $con->prepare("SELECT * FROM $tabela22 WHERE STATUS = 'ATIVO'");
	$sql_conta->execute();
...
?>
	...
	<br>
	<a class="btn btn-app" href="posicoes_trafficmonsoon.php" title="Quantidade de PENDENTES">
	<span class="badge bg-yellow"><?php echo $quantreg_pendentes; ?></span>
    <i class="fa fa-align-justify"></i> Quantidade de PENDENTES
     </a> 
	
	<a class="btn btn-app" href="posicoes_trafficmonsoon_ativos.php" title="Quantidade de ATIVOS">
	<span class="badge bg-green"><?php echo $quantreg_ativos; ?></span>
    <i class="fa fa-align-justify"></i> Quantidade de ATIVOS
     </a>
	<a class="btn btn-app" href="https://trafficmonsoon.com/" title="Acesse aqui o site da TrafficMonsoon" target="_blank"> 
		<img src="../../adm_clientes/img/logo-surf.png" width="120" height="40" alt="Acesse aqui o site da TrafficMonsoon"   />
	</a>
	<hr>
	<br> 
	...
	echo "<br><br>"; // Vai servir só para dar uma linha de espaço entre a paginação e o conteúdo
				...
				foreach($sql_verifc as $ln_verifc) { 
					...
					foreach($sql_verifc_2 as $ln_verifc_2) {
						...
						$id_do_patrocinador = $ln_verifc_2->ID_INDICACAO;
					}
					
					$id_posicao = $ln_verifc->ID;
					$id_do_cliente = $ln_verifc->ID_CLIENTE; 
					
					// busca nome do patrocinador desse cliente
					$sql_2 = $con->prepare("SELECT * FROM $tabela3 WHERE ID = '$id_do_patrocinador'");
					$sql_2->execute();
					$sql_verifc_2 = $sql_2->fetchAll(PDO::FETCH_OBJ);
					foreach($sql_verifc_2 as $ln_verifc_2) {
						$nome_cliente_patrocinador = $ln_verifc_2->NOME; 
					}
?>
	<tbody>
        <tr> 
						...
						<td class='class2'> <?php echo $nome_cliente_patrocinador; ?></td> 
						<td class='class2'>
						<form id="form1" name="form1" method="post" action="ativando_solicitacao_trafficmonsoon.php"> 
							...
							<INPUT TYPE="hidden" NAME="id_do_cliente" VALUE="<?php echo $id_do_cliente; ?>">
								<button type="submit" class="btn btn-success btn-sm" title="ATIVAR essa Solicita&ccedil;&atilde;o">ATIVAR</button> 
						</form>
						</td>
						<td class='class2'> 
						<form id="form_cancela_1" name="form_cancela_1" method="post" action="cancelando_solicitacao_trafficmonsoon.php">
							<INPUT TYPE="hidden" NAME="id_posicao" VALUE="<?php echo $id_posicao; ?>">
							<INPUT TYPE="hidden" NAME="id_do_cliente" VALUE="<?php echo $id_do_cliente; ?>">
