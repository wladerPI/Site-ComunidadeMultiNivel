            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Todas Dicas</li>
                    </ol>
                </section>
                <!-- Main content -->
...
<!-- centro -->		 
 <div class="box-header">
		<h3 class="box-title">Todas Dicas Registradas</h3>
</div><!-- /.box-header -->	 
...
?>	
	...
	<section class="content"> 
                    <div class="row"> 
						<div class="col-xs-12"> 
							<i>Ordem Decrescente por data de Cadastro</i>
                            <div class="box"> 
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela15 ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql->execute();
	...
	echo "<br><br>"; // Vai servir só para dar uma linha de espaço entre a paginação e o conteúdo
				...
				foreach($sql_verifc as $ln_verifc) { 
					...
					$sql = $con->prepare("SELECT * FROM $tabela3 WHERE ID = '$ln_verifc->ID_MODERADOR'");
					$sql->execute();
					$sql_mod = $sql->fetchAll(PDO::FETCH_OBJ);
					foreach($sql_mod as $ln_mod) {
						$nome_mod = $ln_mod->NOME;
					}
?>
	<tbody>
        <tr> 
						<td class='class1'><?php echo $ln_verifc->ID; ?></td> 
						<td class='class2'> <?php if ($ln_verifc->ID_MODERADOR == 0) { echo "ADMIN"; } else { echo $nome_mod; } ?></td> 
						<td class='class2'> <?php echo $ln_verifc->TITULO; ?></td>
						<td class='class2'> <?php echo $ln_verifc->VISUALIZACOES; ?></td>
						<td class='class2'> <?php echo $data; ?></td>
						<td class='class2'>  
						<a class="btn btn-app" href="dica_editar.php?id_dica=<?php echo $ln_verifc->ID; ?>" title="Editar essa Dica">
						<i class="fa fa-edit"></i> Editar Dica
						</a> 
						
						<a class="btn btn-app" href="dica_exclui.php?id_dica=<?php echo $ln_verifc->ID; ?>" title="Excluir essa Dica">
						<i class="fa fa-times"></i> Excluir Dica
						</a>
						</td> 
					</tr>	
    </tbody>  	 
