...
if ($passo1 == "" || $passo1 == "0" ) { 
	 ...
	 echo ("<script type='text/javascript'> alert('Antes de visualizar a Dica 2, veja primeiro a Dica 1 !!!'); location.href='iniciar_dicas.php';</script>");
	exit;
}  

// buscando pag2 PopUP 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '14'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_popup = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}	
// buscando pag2 banner 1 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '15'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner1 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada 
	}	
// buscando pag2 banner 2 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '16'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner2 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
		
	}	
// buscando pag2 banner 3 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '17'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner3 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 4 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '18'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner4 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}	
// buscando pag2 banner 5 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '19'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner5 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada 
	}
// buscando pag2 banner 6 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '20'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner6 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 7 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '21'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner7 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 8 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '22'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner8 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 9 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '23'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner9 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 10 
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '24'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner10 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 11
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '25'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner11 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}
// buscando pag2 banner 12
	$sql_anuncios = $con->prepare("SELECT * FROM $tabela17 WHERE ID = '26'");
	$sql_anuncios->execute();
	$res_anuncios = $sql_anuncios->fetchAll(PDO::FETCH_OBJ);
	foreach($res_anuncios as $ln_anuncios) {  
		$pa2_banner12 = html_entity_decode((string)$ln_anuncios->SCRIPT, ENT_QUOTES, 'utf-8'); // assim faz popup ser usada
	}	
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Segunda Dica</li>
                    </ol>
                </section>
...
?>	
	<div id="anuncios_top">
		<div id="top_left">
			<!--- banner 1 --->
			<?php  echo $pa2_banner1;  ?> 
		</div>
		<div id="top_right">
			<!-- banner 2 -->
			<?php  echo $pa2_banner2; 
			?>  
		</div>
		<div id="top_centro">
			 <!-- banner 3  --> 
			<?php  echo $pa2_banner3; 
			?> 
		</div>
		<br style="clear:both;"> 
	...
	<div id="anuncios_left">
		<!--  banner 4 -->  
		<?php echo $pa2_banner4; ?>
		
		<!---  banner 5 --->
		<?php echo $pa2_banner5; ?>
...
?>		
		...
		<form id='form' name='form' method='post' action='iniciar_dicas3.php'>
			<INPUT TYPE='hidden' NAME='passo1' VALUE='<?php echo $passo1; ?>'>
			<INPUT TYPE='hidden' NAME='passo2' VALUE='ok'>   
			<button type='submit' class='btn btn-warning btn-lg'  title='Clique aqui para seguir para o PR&Oacute;XIMO PASSO '>Ir para Terceira PASSO</button> 
		</form>
		</div>
		<br style="clear:both;">
    </div>	
	<div id="anuncios_rodape">
		<div style="float:left;">
			 <!--  banner  6 --> 
			<?php echo $pa2_banner6; ?> 
		</div>
		<div style="float:left;margin:0px 0px 0px 5%;">
			<!-- banner 7 -->
			<?php echo $pa2_banner7; ?>
		</div>
		<div style="float:left;margin:0px 0px 0px 5%;">
			<!-- Banner 8 -->
			<?php echo $pa2_banner8; ?>
		</div>
		<div style="clear:both;margin:0px 0px 0px 5px;">
			<div style="float:left;margin:0px 10px 0px 0px;">
			 <!--  banner 9 -->  
			<?php echo $pa2_banner9; ?>	
			</div>
			<div style="float:right;margin:0px 0px 0px 10px;">
			<!-- banner 10 -->   
			<?php echo $pa2_banner10; ?>
			</div>		
		</div>
		
		
		<!-- banner 11 -->
			<?php echo $pa2_banner11; ?>
		
		<!--  banner 12-->   
		<?php echo $pa2_banner12; ?>
    </div>	
</div>	 
<!-- FIM centro -->						
                </section><!-- /.content -->
