...
if ($email == "") {
	echo "<script type='text/javascript'> alert('O Campo E-Mail \xE9 obrigat\xF3rio!'); location.href='solicitar_senha';</script>";  
	exit;
}	
...
";
    ...
    if (!$mail->send()) {
		$erro = $mail->ErrorInfo; 
		echo "<script type='text/javascript'> alert('Falha no envio do E-Mail, contate de outra forma, por favor! log: $erro'); location.href='solicitar_senha';</script>";	 
        break; //Abandon sending
		exit;
    } else {
		echo "<script type='text/javascript'> alert('Veja sua sehna em seu email!'); location.href='solicitar_senha';</script>";  
		exit;
    } 
