        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side strech">
                ...
                <section class="content-header">
                    <h1> 
                        <small>BLOG, Atualiza&ccedil;&otilde;es e Novidades de todo o Projeto</small>
                    </h1>
                    <ol class="breadcrumb">  
...
<?php
			if ($moderadores == "") {
?>		
				<i style="color:red;font:italic;">Somente Moderadores da ComunidadeMultiN&iacute;vel podem criar t&oacute;pico aqui.</i> 
<?php
			} else { 
?>				
				<form id="criar_topic" name="criar_topic" method="post" action="criar_topico_blog.php">  
					<button class='btn btn-warning btn-lg' style="width:300px; height:50px; font-size:28px;line-height:0px;" title="Criar um T&Oacute;PICO no BLOG"> + Criar Novo T&oacute;pico </button> 
				</form>
			<?php } ?>		
			</div>
			<br style="clear:both;">
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'BLOG' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	
	 

	$sql_topic = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'BLOG' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql_topic->execute();
	$res_topic = $sql_topic->fetchAll(PDO::FETCH_OBJ);
	
	$sql_conta = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'BLOG'");
	$sql_conta->execute();
