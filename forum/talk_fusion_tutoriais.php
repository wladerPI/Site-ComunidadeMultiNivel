            ...
            <aside class="right-side strech">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">  
                        ...
                        <li class="active">TALK FUSION / Tutoriais e D&uacute;vidas </li>
                    </ol>
                </section>
...
<?php } ?>
				<div class="box-header">
					<h1 class="box-title">TALK FUSION / Tutoriais e D&uacute;vidas</h1>
                    <div class="box-tools pull-right">
						<button class="btn btn-primary btn-sm" data-widget="collapse"><i class="fa fa-minus"></i></button> 
                    </div>
                </div>
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'TALKFUSION' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	
	 

	$sql_topic = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'TALKFUSION' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql_topic->execute();
	$res_topic = $sql_topic->fetchAll(PDO::FETCH_OBJ);
	
	$sql_conta = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'TALKFUSION'");
	$sql_conta->execute();
