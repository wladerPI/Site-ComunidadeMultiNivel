...
<html>
    <head>
        <meta charset="UTF-8">
        <title>trafficMonsoon Como Funciona, ganhar dinheiro, f&oacute;run de discuss&atilde;o - Comunidade MutiN&iacute;vel | F&OacuteRUM D&uacute;vidas, Tutoriais e Conte&uacute;do t&eacute;cnico para nossos afiliados</title>
        <meta content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' name='viewport'>
		<meta name="keywords" content="trafficMonsoon, tutoriais, como funciona, ganhar dinheiro na internet, melhores sites PTC, maiores PTC do mundo, ganhar dinheiro com marketing digital, como ganhar dinheiro na trafficMonsoon, trafficMonsoon &eacute; piramide financeira, divulgar na trafficMonsoon"/>
		<meta name="robots" content="all">
		<Meta Name="Description" Content="TrafficMonsoon a maior empresa de publicidade PTC do mundo, agora est&aacute; adaptada no projeto  da melhor equipe do brasil, aonde estamos disponibilziando at&eacute; o FINANCIAMENTO do crescimento do seu rendimento !!!">
            ...
            <aside class="right-side strech">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">  
                        ...
                        <li class="active">TrafficMonsoon / Tutoriais e D&uacute;vidas </li>
                    </ol>
                </section>
...
<div class="alert alert-warning alert-dismissable">
	...
	<b>Para que voc&ecirc; possa Efetuar uma pergunda ou criar um T&oacute;pico, voc&ecirc; precisa estar Registrado e logado na ComunidadeMultiN&iacute;vel !!!</b><br><br>
	
	<div class="searchform">
<?php
	// buscando url para logar e voltar nessa pagina
	$endereco = $_SERVER ['REQUEST_URI'];
	$separar  = explode('/', $endereco);
...
?>
        <form id="formlogin" name="formlogin" method="post" action="logando.php">
		  <INPUT TYPE="hidden" NAME="url" VALUE="<?php echo $sub3_pagina; ?>">
...
<?php } else { 	?>		
			<div style="float:right;">
				<form id="criar_topic" name="criar_topic" method="post" action="criar_topico.php"> 
					<INPUT TYPE="hidden" NAME="categoria" VALUE="TRAFFICMONSOON"> 
...
<?php } ?>
				<div class="box-header">
					<h1 class="box-title">TrafficMonsoon / Tutoriais e D&uacute;vidas</h1>
                    <div class="box-tools pull-right">
						<button class="btn btn-primary btn-sm" data-widget="collapse"><i class="fa fa-minus"></i></button> 
                    </div>
                </div>
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'TRAFFICMONSOON' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	
	 

	$sql_topic = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'TRAFFICMONSOON' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql_topic->execute();
	$res_topic = $sql_topic->fetchAll(PDO::FETCH_OBJ);
	
	$sql_conta = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'TRAFFICMONSOON'");
	$sql_conta->execute();
	...
	if ($quantreg <= 0) {
		echo "<b style='color:red;'>No momento N&atilde;o temos nenhum T&oacute;pico Registrado</b> <br>";
	}	
...
?>					
						...
						<tr>
							<td  style="width:65%;"><b style="color:#666;font-size:16px;"><a href="topico/<?php echo $ln_topic->ID."-".RemoveAcentos($str); ?>" title="Veja o T&Oacute;PICO"><?php echo $ln_topic->TITULO_TOPICO; ?></a></b><br>
