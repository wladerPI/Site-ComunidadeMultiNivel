            ...
            <aside class="right-side strech">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">  
                        ...
                        <li class="active">ComunidadeMultiN&iacute;vel / Sugest&otilde;es, Cr&iacute;ticas e Coment&aacute;rios </li>
                    </ol>
                </section>
...
<?php } else { 	?>		
			<div style="float:right;">
				<form id="criar_topic" name="criar_topic" method="post" action="criar_topico.php"> 
					...
					<INPUT TYPE="hidden" NAME="subcategoria" VALUE="SUGESTOES">
...
<?php } ?>
				<div class="box-header">
					<h1 class="box-title">Comunidade Multi N&iacute;vel  / Sugest&otilde;es, Cr&iacute;ticas e Coment&aacute;rios</h1>
                    <div class="box-tools pull-right">
						<button class="btn btn-primary btn-sm" data-widget="collapse"><i class="fa fa-minus"></i></button> 
                    </div>
                </div>
...
//######### FIM dados Paginação
	...
	$sql = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'COMUNIDADE' AND SUB_CATEGORIA = 'SUGESTOES' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql->execute();
	$sql_verifc = $sql->fetchAll(PDO::FETCH_OBJ);
	
	 

	$sql_topic = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'COMUNIDADE' AND SUB_CATEGORIA = 'SUGESTOES' ORDER BY ID DESC LIMIT $inicial, $numreg");
	$sql_topic->execute();
	$res_topic = $sql_topic->fetchAll(PDO::FETCH_OBJ);
	
	$sql_conta = $con->prepare("SELECT * FROM $tabela10 WHERE CATEGORIA = 'COMUNIDADE' AND SUB_CATEGORIA = 'SUGESTOES'");
	$sql_conta->execute();
